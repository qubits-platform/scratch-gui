...
test('initialState', () => {
  ...
  expect(
    monitorLayoutReducer(defaultState /* state */, { type: 'anything' } /* action */),
  ).toBeDefined()
  expect(
    monitorLayoutReducer(defaultState /* state */, { type: 'anything' } /* action */).monitors,
  ).toBeDefined()
  expect(
    monitorLayoutReducer(defaultState /* state */, { type: 'anything' } /* action */)
...
test('invalidRect', () => {
  ...
  expect(
    monitorLayoutReducer(reduxState, addMonitorRect(1, { x: 100, y: 100 }, { x: 10, y: 200 })),
  ).toEqual(reduxState)

  // Problem: y end is before y start
  expect(
    monitorLayoutReducer(reduxState, addMonitorRect(1, { x: 100, y: 100 }, { x: 200, y: 10 })),
  ).toEqual(reduxState)
})
...
test('invalidAddMonitorRect', () => {
  ...
  expect(monitorLayoutReducer(reduxState, addMonitorRect(monitorId, upperStart, lowerEnd))).toEqual(
    reduxState,
  )
})
...
test('invalidMoveMonitorRect', () => {
  ...
  expect(
    monitorLayoutReducer(reduxState, moveMonitorRect(monitorId, 1 /* newX */, 1 /* newY */)),
  ).toEqual(reduxState)
  ...
  // Invalid newX
  expect(
    monitorLayoutReducer(reduxState, moveMonitorRect(monitorId, 'Oregon' /* newX */, 1 /* newY */)),
  ).toEqual(reduxState)

  // Invalid newY
  expect(monitorLayoutReducer(reduxState, moveMonitorRect(monitorId, 1 /* newX */))).toEqual(
    reduxState,
  )
})
...
test('invalidResizeMonitorRect', () => {
  ...
  expect(
    monitorLayoutReducer(
      reduxState,
      resizeMonitorRect(monitorId, 'Oregon' /* newWidth */, 1 /* newHeight */),
    ),
  ).toEqual(reduxState)

  // Invalid newHeight
  expect(monitorLayoutReducer(reduxState, moveMonitorRect(monitorId, 1 /* newWidth */))).toEqual(
    reduxState,
  )
