...
describe('themes', () => {
  ...
  describe('block helpers', () => {
    ...
    test('updates extension category based on theme', () => {
      ...
      expect(serializeToString.mock.calls[0][0].documentElement.getAttribute('colour')).toBe(
        '#FFFFFF',
      )
      expect(
        serializeToString.mock.calls[0][0].documentElement.getAttribute('secondaryColour'),
      ).toBe('#DDDDDD')
      expect(serializeToString.mock.calls[0][0].documentElement.getAttribute('iconURI')).toBe(
        'darkPenIcon',
      )
    })
  })
